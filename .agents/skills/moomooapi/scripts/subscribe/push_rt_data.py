...
Function: Subscribe to stock real-time (intraday) data and receive real-time push via Handler
Usage: python push_rt_data.py HK.00700 --duration 60

API limitations:
- Must first subscribe to RT_DATA type, subject to subscription quota limits
"""
import argparse
import json
import time
...
class RTDataHandler(RTDataHandlerBase):
    """Real-time data push callback handler class"""
    ...
    def on_recv_rsp(self, rsp_pb):
        ...
        if self.output_json:
            ...
            print(f"\n[Real-time Data Push] {time.strftime('%H:%M:%S')}")
...
def push_rt_data(codes, duration=60, session_str="NONE", output_json=False):
    ...
    try:
        ...
        check_ret(ret, msg, ctx, "Subscribe to real-time data push")

        if not output_json:
            print(f"Subscribed to real-time data push: {', '.join(codes)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Receive real-time data push")
