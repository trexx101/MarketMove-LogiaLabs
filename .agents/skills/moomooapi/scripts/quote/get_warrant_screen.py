...
def _build_request(market_str, only_count, is_delay, page_from, page_count, spec, simple_args):
    try:
        from moomoo import WarrantScreenRequest
        from moomoo.quote.stock_screen_const import WarrantMarket, WarrantField, WarrantType
    except ImportError as e:
        print(f"错误: 当前 moomoo-api 未提供 WarrantScreenRequest 或相关枚举（{e}）。请升级 SDK。")
        sys.exit(1)
