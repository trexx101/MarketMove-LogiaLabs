...
def _resolve_market_categories(markets):
    try:
        from moomoo import OptMarketCategory
    except ImportError:
        return list(markets)
...
def _build_request(markets, page_from, page_count, spec):
    try:
        from moomoo import OptionScreenRequest, OptUnderlyingIndicator, OptIndicator
    except ImportError:
        print("错误: 当前 moomoo-api 未提供 OptionScreenRequest，请升级 SDK")
        sys.exit(1)
